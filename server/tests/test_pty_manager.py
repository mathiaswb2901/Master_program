...
class TestOneChildIsHandedBackOnce:
    ...
    async def test_a_second_concurrent_release_signals_nothing(self) -> None:
        ...
        assert fake.signals == [True]  # ...and the child was killed exactly once
        assert manager._sessions == {}
    ...
    async def test_one_release_that_raises_does_not_abandon_the_others(self) -> None:
        ...
        assert good.signals == [True], "a sibling's failure took this child's release with it"
        assert bad.signals == []  # it did fail — that is the point of the fixture

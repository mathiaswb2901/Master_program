 ...
 * the user already has running, a document already open somewhere else, a launch
 * that failed, an embed that was refused. There is no state in which this
 ...
 * PowerPoint that cannot dock because the user has one open: the card names the
 * one action that changes the answer, and the note above the fallback comes from
 * the server's `kind_notes` rather than being re-derived here.
 *
...
export function OfficeDocument({ file }: { file: OpenFile }) {
  ...
  // specific to it (PowerPoint already running) rather than the machine-wide one
  // already in `capabilities.detail`. Read from the server, never re-derived
  // here: the condition is a fact about the user's machine, not a UI rule.

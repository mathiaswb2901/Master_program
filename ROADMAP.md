...
- **Office host panels**: real Word/Excel/PowerPoint docked into dockview via native
  ...
  **Composition is now part of this sequence's definition of done** (owner, 2026-08-05:
  ...
  hang containment, which stops being an improvement and becomes a **prerequisite**: one
  wedged guest costs ~1 s per resize frame, so with two docked it is every layout
  interaction in the window. Sequenced as its own PR after Word (#38), the mover thread,
  the COM bridge and Excel — it is the acceptance demo for the whole Office sequence.

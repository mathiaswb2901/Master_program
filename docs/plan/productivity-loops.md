...
all five items A–E to be implemented — into five disjoint, fake-first, independently
shippable PRs with explicit file ownership, so parallel lanes never collide.
...
**2. Consequence: PR-D does not depend on the staged-review PR2 lane.** The brief sequences
PR-D after SR-PR2 because SR-PR2 rewrites `build_context_bridge`'s tool selection and
`_AUTO_ALLOWED` in `services/sdk_factory.py` and touches `services/permission_broker.py`
(`docs/plan/staged-review.md` §"PR 2 — owned files"). With correction 1, PR-D touches
neither file. The ordering constraint it really has is **#122**
(`fix/activity-workspace-reroot`), which owns `services/activity.py` — the file PR-D's one
server-side edit lands in. PR-D rebases onto #122; SR-PR2 is irrelevant to it.
...
widening and it is paid for by the approval below, which is precisely the "explicit one-time
trust prompt, which is a feature" that both `services/gates.py::workspace_config_refusal`
...
### The approval is keyed to the spec's content hash

The permission-broker pattern, applied to a document instead of a tool call:
...
- Approving one records `blake2b(spec bytes)` → `{approver, timestamp}` in the machine's
  app-data dir (**not** in `.workbench/`, for the reason `RecentsStore` spells out: a trust
  decision belongs to the person, and a trust record inside the folder it authorises is a
  trust record an attacker can write).
- **Editing the spec revokes it.** The hash changes, the spec is unapproved again, and the
  panel says so. A one-time approval that survived edits would be a way to get a `callable`
  approved and then swapped.
...
- **Models added:** `ReconcileSpecFile`, `SpecCheck`, `SpecApproval`, `SpecRunReport`,
  `SpecState` (the panel's list), `SpecApprovalRequest`.
- **Test story.** Compilation: a spec becomes a `ReconciliationSpec` whose `value_unit`
  ...
  security half, three ways: a spec that has never been approved runs **nothing** on
  workspace-open (asserted by a runner spy with zero calls); editing an approved spec by one
  byte revokes it; and a `callable` that raises, hangs or prints 50 MB becomes one `fail`
  evidence line with the setting that raises the ceiling, never an exception that sinks the
  run. Determinism: `FakeSpecRunner` scripts values so the whole watcher→gate→chip path is
  green in CI with no user code executed anywhere.
...
Spec       .workbench/reconcile/dispatch.toml  blake2b 1c7e…  approved 2026-08-07 by mathi
...
Every line comes from a field that already exists or that PR-A adds. Nothing in it is
computed twice. It reaches the user two ways, both of which are the registered-capability
...
- **Test story.** A written result reads back byte-identical through the model round trip; a
  ...
  asserts the report names the workbook, the spec hash, the source provenance, every check
  and the approver — and that a result with **no** approval says "not approved" explicitly
  rather than omitting the section (AXI shape 2, applied to a human reader).
...
- **Composes with:** #122 (must rebase), #118 (the origin tag — PR-D works without it, and
  writes one spurious layout save per agent open until it lands, which the plan states
  rather than hides), #119 (`store.ts` permission flags — disjoint), #107 (PowerPoint —
...
Five PRs. **A, C, D and E are mutually disjoint and can run as four parallel lanes**; B is
the only one with a hard prerequisite (A, for the reader seam and the `ReadSource` it stamps
on every result B produces).
...
**Shared files, and who touches them.** `ui/src/panels/ReviewPanel.tsx` is touched by A (one
source line), C (the Export action) and the in-flight SR-PR2 lane (findings rendering) —
three append-only additions to different regions of one component, and the plan states the
order **C after A** so the source line exists before the export renders it.
...
**Composition with the open PRs, honestly:**

- **#118 dockview v7** — PR-D wants its origin tag but is not blocked by it (see PR-D above:
  without it, one spurious layout save per agent-driven open).
- **#119 store permission flags** — disjoint from all five.
- **#116 terminal/agent WS lifecycle** / **#121 shell thread discipline** / **#123 monaco
  shared model** — disjoint from all five.
- **#122 activity workspace re-root** — **PR-D rebases onto it.** It is the only hard
  ordering constraint against an open PR in this plan.
- **#113 voice seam** — disjoint, and a natural later customer for PR-E: a voice command that
  resolves to `layout.switch{name}` is a parameterised invocation, which is why PR-E's
  validation lives in the relay rather than in the CLI.
- **#114 gate-closure check** — disjoint; a third `ValidationCheck` whose results PR-C
  persists and PR-C's export renders without knowing what it is.
- **#107 PowerPoint hosting** — disjoint; PR-D's locus pass covers a slide index for free
  once it lands.
- **The staged-review PR2 lane** (`m6/staged-review-adversarial`, no PR number yet) — it
  rewrites `services/sdk_factory.py` and touches `services/permission_broker.py`. Per
  correction 2, **no PR here touches either file**, so the two tracks are independent. If
  PR-D had been built to the brief's `PostToolUse` design, it would not have been.

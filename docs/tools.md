...
| `onDockReady` | The live `DockviewApi`, once, when the dock exists (and `null` when it goes away) — for a tool that operates on the dock rather than living in it. Two do: the layout system (restore, persistence, focus mode) and the pane system (the popout lifecycle). Anything else should be reaching for `openPanel`. |
